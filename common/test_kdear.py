...
from common.get_ieng_headers import get_ieng_headers
from common.get_ieng_wx_headers import get_ieng_wx_headers
from common.get_ieng_teacher_headers import get_ieng_wx_teacher_headers
from common.get_ieng_agent_headers import get_ieng_wx_agent_headers
    ...
    def test_all_api(self):
        ...
        for i in range(self.m, self.row):
            ...
            if self.session =='ieng':
                ...
                headers = {}
                headers["Content-Type"] = "application/json; charset=UTF-8"
                wx_cookie = get_ieng_wx_headers(url_openid_i)
                ...
                wx_cookie = get_ieng_wx_teacher_headers(url_openid_i)
                print(wx_cookie)
                headers["Cookie"] = wx_cookie
                if datatype_i == 1 or datatype_i == "1":  # datatype_i为1时，接口请求时，必须转换为json对象，即string类型
                    data_i = json.dumps(
                ...
                headers["Content-Type"] = "application/json; charset=UTF-8"
                headers["User-Agent"] = "Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/39.0.2171.95 Safari/537.36 MicroMessenger/6.5.2.501 NetType/WIFI WindowsWechat QBCore/3.43.901.400 QQBrowser/9.0.2524.400"
                wx_cookie = get_ieng_wx_agent_headers(url_openid_i)
                print(wx_cookie)
                headers["Cookie"] = wx_cookie
                if datatype_i == 1 or datatype_i == "1":  # datatype_i为1时，接口请求时，必须转换为json对象，即string类型
                    data_i = json.dumps(
...
#     path_case = os.path.dirname(os.getcwd()) + '/excel_case/iEng_family_test.xlsx'
#     print(path_case)
#     session = 'ieng_wx'
#     row = 2
#     test_kdear(path_case,session,row).test_all_api()