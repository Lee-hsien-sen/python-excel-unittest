...
class test_mock(unittest.TestCase):
    ...
    def test_all_api(self):
        ...
        print(path_case)
        session = 'crm'
        row = 1 # 如果self.row小于2时，则按excel中实际数据行数请求接口，，如果self.row大于等于2时，则按传入的self.row的值取数据
    ...
    # def no_test_html_dingding(self):  # test_html_dingding前面加个no就不会跑这段代码了
        ...
        html_path, pass_count, fail_count = write_html(excel_report_file, "Sheet1", "Name", "CRM接口测试报告", excel_case_time)
        ...
        dingding_data = {"msgtype": "link", "link": {"text": "", "title": "CRM测试环境接口监听报告(执行自192.168.1.66服务器)", "picUrl": "", "messageUrl": ""}}
        dingding_data["link"]["text"] = "CRM测试环境接口监听报告：用例总数" + str(pass_count + fail_count) + "条，成功" + str(
            pass_count) + "条，失败" + str(fail_count) + "条。"
        dingding_data["link"]["messageUrl"] = "http://" + myaddr + "/Html_report/" + html_path
